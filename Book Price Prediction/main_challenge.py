...
scaled_training_features = preprocessing.scale(training_features)
print(scaled_training_features[0])

# RIMOZIONE COLONNA AUTORI, RITENUTA INUTILE SENZA OPPORTUNO PREPROCESSING
scaled_training_features = scaled_training_features[:,1:]
...
predictions = model.predict(scaled_training_features)

agg = 0
for i in (range(len(predictions))):
    agg += pow(math.log10((predictions[i]+1)/training_target[i]),2)
score2 = 1-math.sqrt(agg / (2*len(predictions)))
